...
public class APfinder_5h{
 ...
 public APfinder_5h(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
...
for (int z=1;z<m_in.zdim-1;z++){
  ...
  if (iterate){
	...
	for (int y=0;y<m_in.ydim;y++){
		for (int x=0;x<m_in.xdim;x++){
			if (m_out.dat.get(z,y,x)==0){
				if (countNeighboursOverThreshold(m_out.dat,z,y,x,radius,R)>ca_threshold) frame.dat.set(0,y,x,R);
			}
	  }
    }
