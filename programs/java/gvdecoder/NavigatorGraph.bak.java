...
class NavigatorGraph extends JPanel implements KeyListener, FrameListener{
   ...
   static final int SCALE_INVERSE_MAXDIST=200; //if scaled past this distance, rescale all points.
	...
	int cols;
	int SelectedRoi=-1;
	trace.Cursor SelectedCursor=null;
	...
	Trace DetectorTagReferenceTrace;
    ...
    public void initializeNavigator(String absolutefilename,
	 ...
	 trace.getArrayFromFile gaff=new trace.getArrayFromFile(absolutefilename);
  ...
  public void rescale(){
	...
	public void rescaleToSelection(){
	   ...
	   else
	   {
        ...
        //report these values to Viewer2 if it exists
		if (vw!=null){
	...
	if (CursorVector==null) outfile.writeInt(0);
		...
		trace.Cursor tmp=(trace.Cursor)CursorVector.elementAt(i);
   ...
   public void runDetector(){
    ...
    public void addCursor(int position){
		CursorController cc=null;
			...
			trace.Cursor cursor = new trace.Cursor(filename,CursorVector.size(),(double)tg.raw_x[0],0.0,0.0,position,Color.green,true);
    ...
    public void removeCursor(){
	 ...
	 if ((CursorVector!=null)&&(SelectedCursor!=null)){
	 	...
	 	for (int i=0;i<CursorVector.size();i++){
			//renumber
			((trace.Cursor)CursorVector.elementAt(i)).number=new Integer(i);
	    }
	 }
	repaint();
    ...
    public void paintComponent(Graphics gOld) {
		...
		for (int j=1;j<cols;j++) {
	     ...
	     g.drawPolyline(tg.x_values,tg.getYs(j-1),length);
	    }
	   } else
	   {
		 if (SelectedRoi>0){
		...
		for (int j=0;j<CursorVector.size();j++){
	     trace.Cursor cursor=(trace.Cursor)(CursorVector.elementAt(j));
	     cursor.paintCursor(g,RelativeStart,x_range,x_scale,windowSize,GenericWidth,height);
	     }
        }//!null
...
class sgListener extends MouseInputAdapter {
	...
	for (int j=0;j<sg.CursorVector.size();j++){
	 trace.Cursor tmp=(trace.Cursor)sg.CursorVector.elementAt(j);
