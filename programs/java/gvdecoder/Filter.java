...
public void addLookupFilter(int[] lt){
   GeneralFilter gf=null;
   if (filters.size()==0) gf=new LookupFilter(bi_ref,lt);
    else gf=new LookupFilter(((GeneralFilter)filters.get(filters.size()-1)).getOp(),lt);
   filters.add(gf);
   fireContentsChanged(this,0,filters.size());
}

public void addColorFilter(int[][] lt){
   GeneralFilter gf=null;
   if (filters.size()==0) gf=new ColorFilter(bi_ref,lt);
    else gf=new ColorFilter(((GeneralFilter)filters.get(filters.size()-1)).getOp(),lt);
   filters.add(gf);
   fireContentsChanged(this,0,filters.size());
...
public void addFilter(String filtername){
 ...
 if (filtername.equals("lookup")){
   if (filters.size()==0) gf=new LookupFilter(bi_ref,null);
   else gf=new LookupFilter(((GeneralFilter)filters.get(filters.size()-1)).getOp(),null);
 }else
  ...
  if (filtername.equals("colour")){
    if (filters.size()==0) gf=new ColorFilter(bi_ref,null);
    else gf=new ColorFilter(((GeneralFilter)filters.get(filters.size()-1)).getOp(),null);
 }else
...
public Filter(){
	filters=new ArrayList();
	bc=BorderExtender.createInstance(BorderExtender.BORDER_COPY);
...
/*
 ...
 the memory leak is probably in go(), as it creates an image that may persist.
 I want to test it by drawing this image without the createinstance method call.
 I need to find out how to draw a RenderedOp.
*/
/*
...
//check if pb3 is null
 if (pb3==null){
	...
	pb3 = new ParameterBlock();
	    	...
	    	pb3.add(0.0F);                       // The y translation
	   	pb3.add(Interpolation); // The interpolation
     }catch(Exception e){ System.out.println("\n\n\n Filter failed: Background not present. A file called "+bg_img_name+ " must first be created.\n\n");}
	}
...
BufferedImage bim= im4.createInstance().getAsBufferedImage();
im4.dispose();
return bim;
}
