...
public class Matrix extends ImageDecoderAdapter implements PixelGetter{
    ...
    public double getPixelValue(int z, int y, int x){
		return dat.get(z,y,x);
	}
	public int getZDim(){return zdim;}
    public int getYDim(){return ydim;}
    public int getXDim(){return xdim;}
    public double getXMin(){return (double)0;}
	public double getXMax(){return (double)xdim;}
	public double getYMin(){return (double)0;}
	public double getYMax(){return (double)ydim;}
	public double getZMin(){return (double)0;}
    public double getZMax(){return (double)zdim;}
    public void update(){}
...
/*corrected zyx*/
	public boolean create(int zdim, int ydim, int xdim){

     this.xdim=xdim;
	 this.ydim=ydim;
	 this.zdim=zdim;
	 try{
	 if (!((dat!=null)&&(dat.xdim==xdim)&&(dat.ydim==ydim)&&(dat.zdim==zdim)&&(dat.arr!=null))) { System.out.println("Matrix: generated a new data array object");dat=new doubleArray3D(zdim,ydim,xdim);}
	 if (!((frame!=null)&&(frame.xdim==xdim)&&(frame.ydim==ydim)&&(frame.arr!=null)))  frame=new doubleArray3D(ydim,xdim);
     }catch(Exception e){System.out.println("Matrix create: out of memory error"); return false;}
	 System.out.println("Matrix: called create with "+zdim+" "+ydim+" "+xdim);
	 traces=new TraceManager(this);
	 return true;
	}
...
/*corrected zyx*/
   public void initialize(ImageDecoder id, int start, int end, int ystart, int yend, int xstart, int xend){
			...
			if (create((end-start),yend-ystart,xend-xstart)) { //xdim and ydim are set for the array
    ...
    public void initialize(ImageDecoder id, int start, int end, int ystart, int yend, int xstart, int xend, int bin){
		    int[] tmp=new int[4];
			id.ReturnXYBandsFrames(tmp,instance);
			int[] tmpframe=new int[tmp[0]*tmp[1]];
			int xbins=(int)((xend-xstart)/(1.0*bin));
			int ybins=(int)((yend-ystart)/(1.0*bin));
			if (create((end-start),ybins,xbins)){ //xdim and ydim are set for the array
			for (int i=start;i<end;i++){
			   id.JumpToFrame(i,instance);
			   id.UpdateImageArray(tmpframe,tmp[0],tmp[1],instance);
			   // set((i-start),tmpframe);
			   for (int p=0;p<ybins;p++){
				 for (int q=0;q<xbins;q++){
				  double sum=0;
				  for (int m=0;m<bin;m++){
					  for (int n=0;n<bin;n++){
					    int y=p*bin+m+ystart;
					    int x=q*bin+n+xstart;
					    sum+=tmpframe[y*tmp[0]+x];
					    //dat.set(i-start,p,q,tmpframe[y*tmp[0]+x]);
				      }//m
			         }//n
			        dat.set(i-start,p,q,sum);
				}//q
		     }//p
	       }//i
	   }
	  ...
	  public void initialize(ImageDecoder id, int start, int end, int ystart, int yend, int xstart, int xend, int bin, int tbin){
	 		    int[] tmp=new int[4];
	 			id.ReturnXYBandsFrames(tmp,instance);
	 			int[] tmpframe=new int[tmp[0]*tmp[1]];
	 			int zbins=(int)((end-start)/(1.0*tbin));
	 			int xbins=(int)((xend-xstart)/(1.0*bin));
	 			int ybins=(int)((yend-ystart)/(1.0*bin));
	 			if (create(zbins,ybins,xbins)){ //xdim and ydim are set for the array
	 			Matrix buff=new Matrix(1,ybins,xbins);
	 			for (int i=start;i<end;i++){
	 			   id.JumpToFrame(i,instance);
	 			   id.UpdateImageArray(tmpframe,tmp[0],tmp[1],instance);
	 			   // set((i-start),tmpframe);
	 			   for (int p=0;p<ybins;p++){
	 				 for (int q=0;q<xbins;q++){
	 				  ...
	 				  for (int m=0;m<bin;m++){
	 			        ...
	 			        buff.dat.set(0,p,q,sum+buff.dat.get(0,p,q));
	 				}//q
	 		     }//p
	 		     if (i%tbin==0){
					 for (int pp=0;pp<ybins;pp++){
						 for (int qq=0;qq<xbins;qq++){
							 dat.set((int)(i/tbin),pp,qq,buff.dat.get(0,pp,qq));
				   }
			       }
				  buff.dat.constant_multiply(0);
				 }
	 	       }//i
	 	   }
	   }
	  ...
	  public void initializemedian(ImageDecoder id, int start, int end, int ystart, int yend, int xstart, int xend, int bin, int tbin){
	 		    int[] tmp=new int[4];
	 		    int[] nbhd=new int[bin*bin];
	 			id.ReturnXYBandsFrames(tmp,instance);
	 			int[] tmpframe=new int[tmp[0]*tmp[1]];
	 			int zbins=(int)((end-start)/(1.0*tbin));
	 			int xbins=(int)((xend-xstart)/(1.0*bin));
	 				 ...
	 				 for (int q=0;q<xbins;q++){
	 				  ...
	 				  for (int m=0;m<bin;m++){
	 					  for (int n=0;n<bin;n++){
	 					    ...
	 					    int x=q*bin+n+xstart;
	 					    //sum+=tmpframe[y*tmp[0]+x];
	 					    nbhd[cnt]=tmpframe[y*tmp[0]+x];
	 					    cnt++;
	 				      }//m
	 			         }//n
	 			        java.util.Arrays.sort(nbhd);
	 			        buff.dat.set(0,p,q,nbhd[(int)((bin*bin)/2)]+buff.dat.get(0,p,q));
	 				}//q
	 		     }//p
	 		     if (i%tbin==0){
					 for (int pp=0;pp<ybins;pp++){
	   ...
	   public void initialize(Viewer2 vw, int start, int end, int ystart, int yend, int xstart, int xend, int bin){
	   ...
	   public void initialize(Viewer2 vw, ImagePanel jp,  int start, int end, int ystart, int yend, int xstart, int xend, int bin){
		   ...
		   if (create((end-start),ybins,xbins)){ //xdim and ydim are set for the array
		   	  for (int i=start;i<end;i++){
                           ...
                           java.lang.Thread.sleep(500);
					        ...
					        javax.media.jai.iterator.RandomIter iter=javax.media.jai.iterator.RandomIterFactory.create(jp.filtered,null);
		   	   			   // set((i-start),tmpframe);
		   	   			   for (int p=0;p<ybins;p++){
		   	   				 for (int q=0;q<xbins;q++){
		   	   				  double sum=0;
   ...
   public void initialize(ImageDecoder id, int start, int end){
		...
		id.ReturnXYBandsFrames(tmp,instance);
		int[] tmpframe=new int[tmp[0]*tmp[1]];
		if (create((end-start),tmp[1],tmp[0])){
		for (int i=start;i<end;i++){
		   id.JumpToFrame(i,instance);
		   id.UpdateImageArray(tmpframe,tmp[0],tmp[1],instance);
		   set((i-start),tmpframe);
...
public void initializefiltered(Viewer2 vw, int zstart, int zend, int ystart, int yend, int xstart, int xend){

	create(zend-zstart,yend-ystart,xend-xstart);
	for (int z=zstart;z<zend;z++){
		for (int y=ystart;y<yend;y++){
			for (int x=xstart;x<xend;x++){
				dat.set(z-zstart,y-ystart,x-xstart,vw.getPixel(z,y,x));
			}
		}
	}
...
//   dbg+="setting matrix tracemanager";
  ...
  changed.set(z*(ydim*xdim)+y*(xdim)+x);
}
...
public Matrix __mul__(Matrix ma){
	if ((ma.xdim==xdim)&&(ma.ydim==ydim)){
	if (ma.zdim==1){
	    dat.frame_multiply(ma.dat);
	    dbg="frame_multiply";
    }else
	if (ma.zdim==zdim) dat.multiply(ma.dat);
	else
	dat.frame_multiply(ma.getFrameAverage().dat);
    }
	return this;
...
public Matrix __div__(Matrix ma){
	if ((ma.xdim==xdim)&&(ma.ydim==ydim)){
	if (ma.zdim==1){
	    dat.frame_divide(ma.dat);
	    dbg="frame_multiply";
    }else
	if (ma.zdim==zdim) dat.divide(ma.dat);
	else
	dat.frame_divide(ma.getFrameAverage().dat);
    }
	return this;
...
/** in place subtraction of two matrices **/
 ...
 public void copyframe(Matrix m_in, int z_from, int z_to){
	 for (int y=0;y<ydim;y++){
		 for (int x=0;x<xdim;x++){
			 dat.set(z_to,y,x,m_in.dat.get(z_from,y,x));
		 }
	 }
 }
...
public void set(ROI roi, int z, double d){

	for (int i=0;i<roi.arrs.length;i++){
		int ii=roi.arrs[i];
		int yi=(int)(ii/xdim);
		int xi=(int)(ii-yi*xdim);
		dat.set(z,yi,xi,d);
	}
...
/**
setupProcessRois, processRois, showRois:
Populates global variables roidata and roipixels.
These methods assume (and doesn't check) that the matrix object has a viewer2 object assigned, and there are ROIs selected.
setupProcessRois must be called first to initialize the arrays (not checked).
processRois:
 roipixels (which contain all the indices of pixels in the ROIs)
 roidata (which contain the summed intensities for each roipixel
showRois should be called if the results are to be displayed in a navigator

If endframe<startframe then the array is treated as a circular buffer, with the first index being startframe
     if the last frame with new data is i, then startframe should be i+1, and endframe should be i.
**/

int[][] roidata;
int[][] roipixels;
public void setupProcessRois(int length){
	 roipixels=new int[vw.jp.rois.size()][];
	 for (int i=0;i<vw.jp.rois.size();i++){
	 	 ((ROI)vw.jp.rois.get(i)).findAllPixels(vw.X_dim,vw.Y_dim);
	 	  roipixels[i]=((ROI)vw.jp.rois.get(i)).arrs;
		}
	roidata=new int[vw.jp.rois.size()+1][length];
}


public void processRois(int startframe,int endframe){
     int totalframes;
     int k;

	 if (endframe<startframe){ totalframes=zdim-startframe+endframe+1;}
	 else totalframes=endframe-startframe;

      for (int z=0;z<totalframes;z++){
		  k=(z+startframe)%zdim;
	  //for (int k=startframe;k<endframe;k++){//for each frame
	       roidata[0][z]=z; //the time index

	       for (int i=0;i<roipixels.length;i++){ //for each roi
             roidata[i+1][z]=0;
	         for (int j=0;j<roipixels[i].length;j++){ //go to each element in the roi
	           roidata[i+1][z]+=(int)dat.arr[(k*(xdim*ydim)+roipixels[i][j])];
...
public void showRois(boolean newnavigator){
	vw.roi_t=roidata;
	vw.showRois(newnavigator);
}

/** reorders frames where the data has been used as a circular buffer**/
public void reorder(int endframe){
	int k=0;
    ...
    **/
    public int OpenImageFile(String filename){
		try{
			...
			if (version==2){
				byte[] tmp=new byte[xdim*ydim];
     			 for (int z=0;z<zdim;z++){
				 	  buffer.get(tmp,0,xdim*ydim);
				 				 for (int y=0;y<ydim;y++){
				 					 for (int x=0;x<xdim;x++){
				 						 dat.set(z,y,x,(double)(tmp[y*xdim+x]& 0xFF));
				 					 }
				 				 }
			 }
...
public void SetMask(Matrix ma){
	mask=new boolean[ydim][xdim];
	for (int y=0;y<ydim;y++){
		for (int x=0;x<xdim;x++){
			if (ma.dat.get(0,y,x)>0) mask[y][x]=true;
			//if (ma.dat.get(0,y,x)==0) mask[y][x]=false;
		}
      }
}
...
/***********************************************************************************
counts neighbours in one z plane based on a number of criteria
************************************************************************************/
...
boolean[][] _distancemask;
public void MakeDistanceMask(int r){
	_distancemask=new boolean[2*r+1][2*r+1];
	for (int y=-r;y<=r;y++){
		for (int x=-r;x<=r;x++){
			if (distance(x,y)<r){ _distancemask[y+r][x+r]=true;
		}
	}
 }
}
...
public double RatioOverThreshold(int z, int y, int x, int r, double threshold){
	double sumover=0;
	double sumunder=0;
	if ((_distancemask==null) || (_distancemask[0].length!=2*r+1)) MakeDistanceMask(r);
	if (mask==null) InitializeMask(true);
    for (int yy=-r;yy<=r;yy++){
	 for (int xx=-r;xx<=r;xx++){
	   int yi=y+yy;
	   int xi=x+xx;
       if ((yi>=0) && (xi>=0) && (yi<ydim) && (xi<ydim)){ //in bounds
        if ((_distancemask[yy+r][xx+r])&&(mask[yi][xi])){ //within a certain radius, and not masked out
          if (dat.get(z,yi,xi)>threshold) sumover+=1;
          else sumunder+=1;
        }
      }
    }
  }
 if ((sumunder==0)&&(sumover>0)) return 1;
 if ((sumunder==0)&&(sumover==0)) return 0;

 return sumover/sumunder;
}

public double RatioEqualsThreshold(int z, int y, int x, int r, double threshold){
	double sumover=0;
	double sumunder=0;
	if ((_distancemask==null) || (_distancemask[0].length!=2*r+1)) MakeDistanceMask(r);
	if (mask==null) InitializeMask(true);
...
public Matrix VolumeFilter(double thr, double thrtotal, double thrback, double thrfront){
	...
	if (!res.create(zdim,ydim,xdim)) return null;
	for (int z=1;z<zdim-1;z++){
	 for (int y=1;y<ydim-1;y++){
	   for (int x=1;x<xdim-1;x++){
		   double totalsum=0;
