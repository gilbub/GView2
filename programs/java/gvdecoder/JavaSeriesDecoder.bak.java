...
//gets the series in alphabetical order, puts each in a list
 ...
 System.out.println("the file information is "+dir);
 String[] tmp=null;
 ...
 for (int i=0;i<tmp.length;i++){
  if (tmp[i].endsWith(filetype)) images.add(tmp[i]);
  }
}
  System.out.println("ending OpenImageFile with "+images.size()+" images in directory");
 return 1;
}
...
public int ReturnXYBandsFrames(int[] arr){
	...
	PlanarImage loadImage = JAI.create("fileload", fileName);
