...
public class BigRedShirtDecoder extends ImageDecoderAdapter {
...
public BigRedShirtDecoder(){

}

public BigRedShirtDecoder(int averageover){
...
public int OpenImageFile(String filename){
 ...
 try{
  ...
  bytebuffer=new byte[FRAMESTEP*2];
     ...
     for (int x=0;x<xdim;x++){
	  for (int y=0;y<ydim;y++){
	   ...
	   for (int k=0;k<FRAMESTEP;k++){
		int index=5120+  2*iNumFrames*(y*xdim+x) + (FrameNumber*FRAMESTEP+k)*2;
		Picfile.seek(index);
	    sum+=getRedShirtShort(Picfile);
	  }
	  arr[y*xdim+x]=sum;
    }
    }
    ...
    try{
     for (int x=0;x<xdim;x++){
	  for (int y=0;y<ydim;y++){
	   int sum=0;
	 //  for (int k=0;k<FRAMESTEP;k++){
		int index=5120+  2*iNumFrames*(y*xdim+x) + (FrameNumber*FRAMESTEP)*2;
		Picfile.seek(index);
		Picfile.read(bytebuffer);
	    for (int k=0;k<FRAMESTEP;k++){

	    sum+=getRedShirtShort(bytebuffer[k*2],bytebuffer[k*2+1]);
...
int getRedShirtShort(byte b0, byte b1){
  int tmp=((b1<<8) +  b0);
  if (tmp>=32768) tmp-=65536;
  return tmp;
...
public int SumROIs(int[][] rois, String outfile, int startframe, int endframe,int instance){
     ...
     System.out.println("in redshirt SumROIs start="+startframe+" end="+endframe);
  ...
  try{
     for (int i=0;i<rois.length;i++){ //for each roi
       ...
       for (int j=0;j<rois[i].length;j++){ //go to each element in the roi
         int yindex=(int)(rois[i][j]/framewidth);
         int xindex=rois[i][j]-yindex*framewidth;
         for (int z=startframe;z<endframe;z++){
		  int summed=0;
          for (int k=0;k<FRAMESTEP;k++){
          int index=5120+  2*iNumFrames*(yindex*framewidth+xindex)+(z*FRAMESTEP+k)*2;
		  Picfile.seek(index);
          summed+=getRedShirtShort(Picfile);
	     }//k
         sum[z-startframe][i]+=summed;
       }//j
       //System.out.println("sim for frame "+k+" = "+sum[k-startframe][i]);
      }//i
     }//
   }catch(Exception e){System.out.println("difficulty reading source file");}
    //generate output
    try{
