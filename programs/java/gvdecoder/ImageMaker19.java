...
public void setIndexArray(int elements, int groupsize, long seed, boolean shuffle){
	int si,v1,v2,ii;
...
public void encode(BufferedImage image, int subcount, int t_index, BufferedImage bi_out){
 ...
 for ( x=0;x<s_xdim;x++){
   for ( y=0;y<s_ydim;y++){
	 ...
	 if (ci<0){cineg=true;badindex=index;}
	 m_y=(int)((double)ci/subcount);
	 m_x=ci-(m_y*subcount);
	 if (bi_out.getRGB(xi+m_x,yi+m_y)!=black){
         colide=true;
	 }
     bi_out.setRGB(xi+m_x,yi+m_y,image.getRGB(xi+m_x,yi+m_y));
 }
...
public Delaunay_Triangulation dt;
Point_dt[] dts;
Triangle_dt tri;
int pxb=0;
int pxe=0;
	...
	for ( x=sx;x<ex;x++){
		for ( y=sy;y<ey;y++){
		     ...
		     for (i=-w;i<(w+1);i++){
			  ...
			  dts=new Point_dt[list.size()];
			  for (int k=0;k<list.size();k++){ dts[k]=(Point_dt)list.get(k);}
			  dt=new Delaunay_Triangulation(dts);
			  tri=dt.find(new Point_dt(x,y));
			  ...
			  getRGBi(c2,rgb_2);
			  getRGBi(c3,rgb_3);
			  Triangle_dt redt  =new Triangle_dt(new Point_dt(x1,y1,(double)rgb_1[0]),new Point_dt(x2,y2,(double)rgb_2[0]),new Point_dt(x3,y3,(double)rgb_3[0]));
			  Triangle_dt greent=new Triangle_dt(new Point_dt(x1,y1,(double)rgb_1[1]),new Point_dt(x2,y2,(double)rgb_2[1]),new Point_dt(x3,y3,(double)rgb_3[1]));
			  Triangle_dt bluet =new Triangle_dt(new Point_dt(x1,y1,(double)rgb_1[2]),new Point_dt(x2,y2,(double)rgb_2[2]),new Point_dt(x3,y3,(double)rgb_3[2]));
			  ri=redt.z(x,y);
			  gi=greent.z(x,y);
			  bi=bluet.z(x,y);
			  //transparent color
			  image.setRGB(x,y,RGBtoPix((int)ri, (int)gi, (int)bi));
              if ((x>=pxb) && (x<pxe) && (y>=pyb) && (y<pye)){
                  ...
                  System.out.println("ri,gi,bi "+ri+","+gi+","+bi);
				  System.out.println("");
...
double ri,gi,bi,wi;
ArrayList list;
public void interp_a(BufferedImage image, int sx, int sy, int ex, int ey, int w){
	int[] rgb_1=new int[3];
	int[] rgb_2=new int[3];
	int[] rgb_3=new int[3];
    int xdim=image.getWidth();
		...
		for ( y=sy;y<ey;y++){
		     ...
		     for (i=-w;i<(w+1);i++){
				 for (j=-w;j<(w+1);j++){
				   if (touched[(y+i)*xdim+(x+j)]){
					   //add this to list of potential pixels
					   list.add(new xyc(x+j,y+i,x,y,image.getRGB(x+j,y+i)));
				   }
				}
			  }
			  ri=0;gi=0;bi=0;wi=0;
			  for (int k=0;k<list.size();k++){
				  xyc mxy=(xyc)list.get(k);
				  ri+=mxy.R*mxy.w;
				  bi+=mxy.B*mxy.w;
				  gi+=mxy.G*mxy.w;
				  wi+=mxy.w;
...
class xyc{
	...
	int G;
	int B;
	public xyc(int x, int y, int x0, int y0, int pixval){
	  this.x=x;
	  this.y=y;
	  this.w=1.0/Math.sqrt( (x0-x)*(x0-x)+(y0-y)*(y0-y) );

	  this.R= (pixval & 0x00FF0000) >> 16;
	  this.G= (pixval & 0x0000FF00) >> 8;
      this.B= pixval & 0x000000FF;
