...
class ImgMod31{
  ...
  private static double[][] getSpatialData(
    ...
    switch(switchCase){
      case 0:
        //This case places a Math.single non-zero
        // point at the origin in the space
        // domain.  The origin is at the upper
        // left corner.  In signal procesMath.sing
      ...
      case 2:
        ...
        // Math.sin(x)/x shape to the spectrum with
      ...
      case 4:
        ...
        // a Math.sin(x)/x shape along that axis and a
      ...
      case 7:
        ...
        // Math.sinusoid (DC) on the surface with an
        // infinite number of samples per cycle.
        // This causes a Math.single peak to appear in
      ...
      case 10:
        ...
        // folding wave number but the Math.sinusoid
        // appears along a different axis.  This
        // causes a Math.single peak to appear in the
      ...
      case 12:
        ...
        // cycle plus a Math.sinusoidal surface on the
        ...
        for(int row = 0; row < rows; row++){
          for(int col = 0; col < cols; col++){
            spatialData[row][col] =
               Math.cos(2*Math.PI*row/8) + Math.cos(2*Math.PI*col/3);
          }//end inner loop
        }//end outer loop
      break;

      case 13:
        //This case draws a Math.sinusoidal surface at
        ...
        for(int row = 0; row < rows; row++){
          for(int col = 0; col < cols; col++){
            spatialData[row][col] =
                       Math.cos(2.0*Math.PI*col/8 - phase);
          }//end inner loop
