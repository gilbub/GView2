...
public class Viewer2 extends JInternalFrame implements KeyListener, ActionListener, ImageViewer, HtmlSaver, FrameListener{
    ViewerParams vp;
...
public void SetFrame(int framenum){
	if (framenum!=frameNumber) JumpToFrame(framenum);
}
  ...
  public void setupactionmap(){
   ...
   public void keyPressed(KeyEvent e) {
    ...
    public RecordControlPanel rc;
    public void SetupRecordWindow(){
        ...
        rc=new RecordControlPanel(this);
        rc.setPreferredSize(new Dimension(300,300));
        MainControlPanel.add(rc);
        viewPane=new JPanel();
		  ...
		  viListener vilistener = new viListener(this);
				  ...
				  popup.add(submenu);
				   ...
				   submenu = new JMenu("view");
							  ...
							  rbMenuItem.addActionListener(myViewMenuListener);
							  submenu.add(rbMenuItem);
							  rbMenuItem = new JRadioButtonMenuItem("2.0x");
							  group.add(rbMenuItem);
							  rbMenuItem.addActionListener(myViewMenuListener);
							  submenu.add(rbMenuItem);
	...
	public void SetupWindow(){
        ...
        /* contains framerate to user def */
...
//				  menuItem.addActionListener(myViewMenuListener);
   ...
   public Viewer2(GView ifd, int fps, String filename, String absolutefilename, ImageDecoder id, boolean recorder){
   		 super(filename,
   					 true, //resizable
   					 true, //closable
   					 true, //maximizable
...
public double realpixelscale=1.0;
	...
	if ((jp.presentruler!=null)&&(jp.presentruler.distance!=0))
	 title=filename+" "+jp.presentruler.distance+" "+jp.presentruler.distance*realpixelscale;
	 else title=filename+" ("+frameNumber+","+lastCoordX+","+lastCoordY+")="+val;
	  ...
	  for (int k=startframe;k<endframe;k++){//for each frame
	       ...
	       for (int i=0;i<roi.length;i++){ //for each roi
	         ...
	         for (int j=0;j<roi[i].length;j++){ //go to each element in the roi
	           sval+=datArray[roi[i][j]];
	         }//j
	           roi_t[i+1][k-startframe]=(int)(sval/roi[i].length*100.0);
 ...
 public int getPixel(int z, int y, int x){
  ...
  public void openNavigator(){
   ...
   if (!file.exists()){
	  int[] tmp=new int[4];
	  im.ReturnXYBandsFrames(tmp,0);
	  int fr=tmp[3];
	  if (fr<1){ javax.swing.JOptionPane.showInternalMessageDialog(this,"A navigator control can't be created.\n"+
	                                                            "Please generate and then save the navigator graph\n"+
	                                                            "(process Rois) first");
			 return;
			}
	 double[] tmp2 = new double[fr];
	 tmp2[0]=0.5;
	 gvdecoder.trace.Trace tr=new gvdecoder.trace.Trace(tmp2);
	 NavigatorGraph nav=new NavigatorGraph("tmp",ifd,tr);
	 nav.vw=this;
     this.nav=nav;
     nav.ISBLANK=true;
	 return;
...
//try to save the image
   ...
   public void saveROIs(){saveROIs("lastROIs.roi");}
   public void saveROIs(String filename){
	  try{
   ...
   public void loadROIs(){loadROIs("lastROIs.roi");}
   public void loadROIs(String filename){
	 try{
	...
	public void setViewScale(){
		...
		viewScale=(float)windowSize.getWidth()/(float)X_dim;
...
*/
    ...
    public void histogram(int[] data, double histmin, double histmax, int bins){
         double dv;
         int i,j;
         double drange=histmax-histmin;
           if (drange<1){
		   System.out.println("histogram range too small, returning");
		   return;
	      }

    	 if ((histdata==null)||(histdata.length!=bins)) histdata=new double[bins];
    	 else
    	  for (i=0;i<histdata.length;i++) histdata[i]=0;
    	 for (i=0;i<data.length;i++){
		   dv=(double)data[i];
		   if (background!=null) dv=dv-(int)background[i];
		   j=(int)((dv-histmin)/drange * bins);
		   if (j>=bins) j=bins-1;
		   if (j<0) j=0;
    ...
    public void dataHistogram(){
	    ...
	    for (int i=0;i<datArray.length;i++){
		  v=datArray[i];
		  if (background!=null) v=v-(int)background[i];
		  if (v>histdatamax) histdatamax=v;
		  if (v<histdatamin) histdatamin=v;
		}
		histogram(datArray,histdatamin,histdatamax,bins);
	  	}
   ...
   public void viewHistogram(){
		...
		histogram(viewArray,histdatamin,histdatamax,bins);
	  	}
	  	 ...
	  	 if (((ROI)jp.rois.get(i)).poly.contains(lastMouseX/viewScale, lastMouseY/viewScale)){
	           ...
	           for (int y=0;y<rect.height;y++){
				   for (int x=0;x<rect.width;x++){
					   ma.dat.set(y,x,(double)datArray[(rect.y+y)*X_dim+(x+rect.x)]);
...
public void itemStateChanged(ItemEvent e){
 ...
 public void actionPerformed(ActionEvent e) {
	...
	else if (sourcetext.equals("2.0x")) {vi.jp.offsetx=0; vi.jp.offsety=0;vi.setVisualScale(2.0f);}
  ...
  public void mouseReleased(MouseEvent e) {
      ...
      if (vi.DRAGMODE==vi.DRAWRULER){
	   //dragmode=drawruler
        ...
        vi.jp.presentruler.notifyRulerCreated();
