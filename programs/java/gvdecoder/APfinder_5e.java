...
public class APfinder_5e{
 ...
 public APfinder_5e(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
...
public void clean(Matrix m_in, Matrix m_out, int spikethreshold, int radius, int R,boolean iterate){
// suppress isolated spikes
double ca_threshold=((radius*2)*(radius*2)/4.0);
for (int z=1;z<m_in.zdim-1;z++){
  ...
  if (iterate){
	for (int y=0;y<m_in.ydim;y++){
		for (int x=0;x<m_in.xdim;x++){
			if (m_out.dat.get(z,y,x)==0){
				if (m_out.dat.countNeighboursOverThreshold(z,y,x,radius,R)>ca_threshold) m_out.dat.set(z,y,x,R-1);
			}
	  }
    }
   }
