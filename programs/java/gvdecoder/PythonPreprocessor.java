...
public class PythonPreprocessor{
 ...
 String python_param_class=null;
 ...
 public boolean fileexists(String classname){
	...
	String filename= "gvdecoder"+File.separator+"anonclasses"+File.separator+classname;
 ...
 public boolean processGUI(String s){
	 parseerror=null;
     parsesuccess=true;
	 ...
	 while(start!=-1){
		 start=s.indexOf("@gui",start+1);
		 end=s.indexOf("\n",start);
		 String funcname=s.substring(start+4,end).trim();
 ...
 public boolean processParameters(String s){
	...
	while (start!=-1){
	 ...
	 if (eq>-1){
		 int endpos=paramstring.length();

		 //find if there is a comment symbol '#'

		 if (paramstring.indexOf("#")!=-1){
			 int tmp=paramstring.indexOf("#");
			 par.comment=paramstring.substring(tmp+1,endpos);
		     endpos=tmp-1;
		 }
		 par.paramname=(paramstring.substring(2,eq)).trim();
		 par.paramvalue=(paramstring.substring(eq+1,endpos)).trim();
		 par.ok=true;
		} else{
		 par.ok=false;
		}
 ...
 for (Parameter param:params){
    if (param.ok)
     pb.append("  "+param.paramname+"="+param.paramvalue+"\n");
 }
 python_param_class=pb.toString();
 return true;
}
 ...
 public boolean processJava(String s){
	 session+=1;
	 int start=0;
	 ...
	 while(start!=-1){
		 ...
		 if (start!=-1){
			 ...
			 if (end!=-1){
			  ...
			  cs.append("package gvdecoder.anonclasses;\n");
