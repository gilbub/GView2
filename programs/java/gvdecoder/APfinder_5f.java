...
public class APfinder_5f{
 ...
 public APfinder_5f(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
...
for (int z=1;z<m_in.zdim-1;z++){
	...
	for (int y=0;y<m_in.ydim;y++){
		for (int x=0;x<m_in.xdim;x++){
			...
			if (m_out.dat.get(z,y,x)==0){
				int count_0=countNeighboursOverThreshold(m_in.dat,z,y,x,radius,10);
				int countm1=countNeighboursOverThreshold(m_in.dat,z-1,y,x,radius,10);
  ...
  if (iterate){
	...
	for (int y=0;y<m_in.ydim;y++){
		for (int x=0;x<m_in.xdim;x++){
			if (m_out.dat.get(z,y,x)==0){
				if (countNeighboursOverThreshold(m_out.dat,z,y,x,radius,R)>ca_threshold) frame.dat.set(z,y,x,R);
			}
	  }
    }
...
public int countNeighboursOverThreshold(doubleArray3D da,int z, int y, int x, int r, double val){
	int sum=0;
	int ybegin=y-r;
	int yend=y+r;
	int xbegin=x-r;
