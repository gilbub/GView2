...
public void decode_hi(BufferedImage image, int subcount, int t_index, BufferedImage bi_out){
  ...
  for ( x=0;x<s_xdim;x++){
	for ( y=0;y<s_ydim;y++){
		 ...
		 m_y=(int)((double)ci/subcount);
		 m_x=ci-(m_y*subcount);
         bi_out.setRGB(x+m_x,y+m_y,image.getRGB(xi+m_x,yi+m_y));
	 }
	 }
...
public void interp(BufferedImage image, int sx, int sy, int ex, int ey, int w){
	...
	for ( x=sx;x<ex;x++){
		for ( y=sy;y<ey;y++){
		     ...
		     for (i=-w;i<(w+1);i++){
				 for (j=-w;j<(w+1);j++){
				   if ((image.getRGB(x+j,y+i)&0xFF000000)>>24!=0){
					   //add this to list of potential pixels
					   list.add(new xy(x+j,y+i));
				   }
				}
			  }
			  Delaunay_Triangulation dt=new Delaunay_Triangulation((Point_dt[])list.toArray());
	...
	for ( x=sx;x<ex;x++){
		for ( y=sy;y<ey;y++){
	       ...
	       if( ((pix & 0xFF000000) >> 24)==0)
	        image.setRGB(x,y,pix+0xFF000000);
	   }
   }
