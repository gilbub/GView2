		 ...
		 for (int z=0;z<matrices[i].zdim;z++){
			 for (int x=0;x<matrices[i].xdim;x++){
				 for (int y=0;y<matrices[i].ydim;y++){
					 dat.set(index,x,y,matrices[i].dat.get(z,x,y));
...
public String lastFileDir=null;
...
public String initialize(String filetype, int start, int end){
	  ...
	  tryDir=gv.pv.getStringProperty("datafiledir",".");
	  gv.fc.setMode(FileDialog.LOAD);
	  gv.fc.setDirectory(tryDir);
   ...
   Vector results=new Vector();
	int z=0;
	 ...
	 for (int y=0;y<ydim;y++){
			 for (int x=0;x<xdim;x++){
	           if (lookover){
	           if (dat.get(z,x,y)<threshold) dat.set(z,x,y,-1);
	             else dat.set(z,x,y,0);
		      }else{
		       if (dat.get(z,x,y)>threshold) dat.set(z,x,y,-1);
			  	             else dat.set(z,x,y,0);
		      }
			  ...
			  do{
				...
				for (int y1=miny;y1<=maxy;y1++){
					for (int x1=minx;x1<=maxx;x1++){
						if (dat.get(z,x1,y1)==0){
							hasmore=true;
							dat.set(z,x1,y1,blobcount);
							blobsize+=1;
