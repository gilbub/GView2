...
public class APfinder_5b{
 ...
 public APfinder_5b(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
...
public double[] findLocalMaxima(double[] arr, int minp,double threshold){
	...
	while (checkspikes){
     ...
     for (int k=1;k<spikes.size();k++){
		 ...
		 for (int p=k-1;p>=0;p--){
		  ...
		  if (nextnonzero!=-1){
			if (spikes.get(k)-spikes.get(nextnonzero)<minp){
				checkspikes=true;
				if (arr[spikes.get(k)]>=arr[spikes.get(nextnonzero)]) spikes.set(nextnonzero,0);
				else spikes.set(k,0);
			}
		}//nextnonzero!=-1
	  }//k
    ...
    for (int m=0;m<spikes.size();m++) {if (spikes.get(m)>0) APStartTrace[spikes.get(m)]=1;}
    return APStartTrace;
}
