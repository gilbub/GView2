...
//ma[i,j][i] = double value, but this is y,x,z instead of x,y,x
//ma[i]=f
//ma.t[1,1] = trace at (1,1)
//ma.t[1] trace # 1
//ma.f[1] = ma.frame
...
public TraceManager __getitem__(PyTuple tup){
  lasty=((PyInteger)tup.__getitem__(0)).getValue();
  lastx=((PyInteger)tup.__getitem__(1)).getValue();
  if ((lasty>ma.ydim)||(lastx>ma.xdim))	throw Py.IndexError("index out of range");

  double[] vals=new double[ma.zdim];
  getPresentTrace(vals,lasty,lastx);
...
public void __setitem__(PyTuple tup, TraceManager tr){
  ...
  if (tr.stack.size()==0){
	for (int z=0;z<ma.zdim;z++){
	  ma.dat.set(z,lasty,lastx,tr.ma.dat.get(z,lasty,lastx));
		 }
	   }
  else{
	 opresult op=(opresult)tr.stack.pop();
      for (int z=0;z<ma.zdim;z++) ma.dat.set(z,lasty,lastx,op.res[z]);
	 }
	tr.stack.clear();
}

public double __getitem__(int i){
	return ma.dat.get(i,lasty,lastx);
}

public void __setitem__(int i, double val){
	ma.dat.set(i,lasty,lastx,val);
}
