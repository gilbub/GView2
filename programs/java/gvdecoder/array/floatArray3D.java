...
public floatArray3D(int y, int x){
	arr=new float[y*x];
	zdim=1;
	xdim=x;
	ydim=y;
...
public floatArray3D(int z, int y, int x){
 arr=new float[z*y*x];
 zdim=z;
 xdim=x;
 ydim=y;
...
public floatArray3D(floatArray3D old){
 //creates a copy
 zdim=old.zdim;
 xdim=old.xdim;
 ydim=old.ydim;
...
public float[][] get2Darray(int zindex){
	float[][] arr=new float[ydim][xdim];
...
public void subtract(floatArray3D old){
	if (old.arr.length==arr.length){

	for (int i=0;i<arr.length;i++){
		arr[i]=arr[i]-old.arr[i];
	}
  }else System.out.println("floatArray3D:: the arrays must be the same length to subtract");
}

public void assign(floatArray3D old){
...
public void setIndex(int index,float val){
	if (index<size)
...
public float get(int z,int y,int x){
...
public void set (int y, int x, float val){
 ...
 public void set (int z, int y, int x, float val){
    index=z*frame+y*xdim+x;
	if ((index<size)&&(index>=0))
	 arr[index]=val;
 }

 public float[] section(int x, int y, int startz, int endz){
	 float[] tmp=new float[endz-startz];
 ...
 public float[] section(int x, int y){
	float[] tmp=new float[zdim];
...
public int fill2D(int z, int y, int x, float empty, float set_to){
	int numset=0;
...
public int fill2DThresh(int z, int y, int x, float empty, float set_to, float thresh, int offset){
...
int xbegin;
int xend;
public int countNeighbours(int z, int y, int x, int r, float val){
	int sum=0;
	ybegin=y-r;
	yend=y+r;
	xbegin=x-r;
...
public int countNeighboursOverThreshold(int z, int y, int x, int r, float val){
	int sum=0;
	ybegin=y-r;
	yend=y+r;
	xbegin=x-r;
...
public void multiply2D(floatArray3D input, int xoffset, int yoffset, floatArray3D result){
...
public int countOverThreshold(float threshold){
	int count=0;
...
public int countOverThreshold(float threshold,int startz, int endz, int startx, int endx, int starty, int endy){
	int count=0;
...
public int changeValue(int z, float oldval, float newval){
    int sum=0;
...
public static void main(String[] arg){
	floatArray3D arr=new floatArray3D(10,10);
	...
	System.out.println("\n count nbs="+arr.countNeighbours(0,7,7,1,6.0f));
	System.out.println("\n count nbs="+arr.countNeighbours(0,7,7,2,6.0f));
