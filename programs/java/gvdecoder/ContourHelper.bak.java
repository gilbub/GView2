...
public class ContourHelper extends JPanel{
 ...
 int nbx=0;
 int nby=0;
 public boolean hasBlobNb(int x, int y, int blob){
	 	boolean found=false;
	 	for (int j=-1;j<=1;j++){
		 for (int i=-1;i<=1;i++){
			 int xn=x+i;
 ...
 public GeneralPath findContour2(double hi, double lo){
	...
	for (int b=1;b<=numblobs;b++){
		...
		//	for (int j=-1;j<=1;j++){
			...
			if (!foundnext){
				//problem??
				System.out.println("cant find next...");
				//search from  right to left
				yn=newy+1;
				for (int k=xn;k>0;k--){
					if (blobs[yn][k]==b){
						System.out.println("found from right "+k+" "+yn);
						blobs[yn][k]=b*-1;
						newy=yn;
						newx=k;
						foundnext=true;
			    ...
			    if (!foundnext) System.out.println(" really cant find next...");
			   }
