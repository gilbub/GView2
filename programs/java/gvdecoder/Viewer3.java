...
public class Viewer3 extends JInternalFrame implements KeyListener, ActionListener, ImageViewer, HtmlSaver{
    ViewerParams vp;
...
//				  menuItem.addActionListener(myViewMenuListener);
				  ...
				  submenu.setMnemonic(KeyEvent.VK_E);
					  ...
					  menuItem = new JMenuItem("save GView3");
					  menuItem.addActionListener(myViewMenuListener);
					  submenu.add(menuItem);
				  popup.add(submenu);
				   ...
				   submenu = new JMenu("view");
							  ...
							  rbMenuItem.addActionListener(myViewMenuListener);
							  submenu.add(rbMenuItem);
							  rbMenuItem = new JRadioButtonMenuItem("toggle move");
							  group.add(rbMenuItem);
							  rbMenuItem.addActionListener(myViewMenuListener);
							  submenu.add(rbMenuItem);
        ...
        popup.add(submenu);
		 ...
		 submenu.setMnemonic(KeyEvent.VK_R);
				  ...
				  menuItem = new JMenuItem("process range");
					menuItem.addActionListener(myViewMenuListener);
					submenu.add(menuItem);
	...
	public Viewer3(GView3 ifd, int fps, String filename, String absolutefilename, ImageDecoder id){
		 super(filename,
					 true, //resizable
					 true, //closable
					 true, //maximizable
   ...
   public Viewer3(GView3 ifd, int fps, String filename, String absolutefilename, String filetype, int frameskip) {
       super(filename,
	                 true, //resizable
	                 true, //closable
	                 true, //maximizable
...
public Viewer3(GView3 ifd, int fps, String filename, String absolutefilename, String filetype) {
    super(filename,
		                 true, //resizable
		                 true, //closable
		                 true, //maximizable
...
public double realpixelscale=1.0;
	...
	if ((jp.presentruler!=null)&&(jp.presentruler.distance!=0))
	 ...
	 else title=filename+" ("+lastCoordX+","+lastCoordY+")="+val;
   ...
   public void findScaleOffsetRoi(){
	 ...
	 try{
	  ...
	  int max=Integer.MIN_VALUE;
			 ...
			 int index=roi.arrs[i];
	  	 	 if (datArray[index]<min) min=datArray[index];
	  	 	 if (datArray[index]>max) max=datArray[index];
	  	 	 }
	  	    }
	  	    if (normalize!=null){
	  			offset=min;
	  	 	 ...
	  	 	 for (int i=0;i<roi.arrs.length;i++){
			 int index=roi.arrs[i];
	  		 int val=(int)(((double)datArray[index]-background[index])*normalize[index]);
	  	 	 if (val<min) min=val;
	  	 	 if (val>max) max=val;
	  	 	 }
	  	    }
...
class MatrixSubAction extends AbstractAction{
	...
	public MatrixSubAction(Viewer3 vw, String txt){
	super(txt);
	this.vw=vw;
	}
...
class ViewMenuListener implements ActionListener, ItemListener{
 ...
 public ViewMenuListener(Viewer3 vi){
  this.vi=vi;
  }
...
public void itemStateChanged(ItemEvent e){
 ...
 public void actionPerformed(ActionEvent e) {
	...
	else if (sourcetext.equals("save GView3")){vi.writeGView3();}
	...
	else if (sourcetext.equals("2.0x")) {vi.setVisualScale(2.0f);}
	else if (sourcetext.equals("1.0x")) {vi.setVisualScale(1.0f);}
	else if (sourcetext.equals("0.5x")) {vi.setVisualScale(0.5f);}
	else if (sourcetext.equals("0.25x")) {vi.setVisualScale(0.25f);}
	else if (sourcetext.equals("auto")) {vi.setVisualScale(0f);}
...
Viewer3 vw;
    ...
    public void aFrameResized(){
	  System.out.println("Viewer3 resize via aframe");
