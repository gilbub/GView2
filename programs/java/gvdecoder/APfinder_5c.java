...
public class APfinder_5c{
 ...
 public APfinder_5c(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
		...
		if (spiketime>0){
		  ...
		  for (int n=spiketime-minp;n<spiketime+minp;n++){
			  if (APStartTrace[n]==1){ // already a spike nearby
				  if (n!=spiketime){
				  if(arr[spiketime]>arr[n]){arr[n]=0;}
				  else{arr[spiketime]=0;}
			   }
			  }
		  }
	    }
