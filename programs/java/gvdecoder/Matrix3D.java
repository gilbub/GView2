...
class Matrix3D {
    ...
    public void transform(Matrix m_in, Matrix m_out){
		...
		for (int z=0;z<m_in.zdim;z++){
			for (int y=0;y<m_in.ydim;y++){
				for (int x=0;x<m_in.xdim;x++){
 					double val=m_in.dat.get(z,y,x);
	                  int nx= (int) (x * lxx + y * lxy + z * lxz + lxo);
	                  int ny = (int) (x * lyx + y * lyy + z * lyz + lyo);
	                  int nz = (int) (x * lzx + y * lzy + z * lzz + lzo);
					  m_out.dat.set(nz,ny,nx,val);
				}
	          }
             }
           }
    ...
    public void floattransform(Matrix m_in, Matrix m_out){
		...
		for (int z=0;z<m_in.zdim;z++){
			for (int y=0;y<m_in.ydim;y++){
				for (int x=0;x<m_in.xdim;x++){
 					double val=m_in.dat.get(z,y,x);
 					if (val>0){
	                  float nx = (x * lxx + y * lxy + z * lxz + lxo);
	                  float ny = (x * lyx + y * lyy + z * lyz + lyo);
	                  float nz = (x * lzx + y * lzy + z * lzz + lzo);
	                  for (int zi=(int)(nz-1);zi<=(int)(nz+1);zi++){
						  for (int yi=(int)(ny-1);yi<=(int)(ny+1);yi++){
							  for (int xi=(int)(nx-1);xi<=(int)(nx+1);xi++){
								  double dist=Math.sqrt( (zi-nz)*(zi-nz)+(yi-ny)*(yi-ny)+(xi-nx)*(xi-nx) );
								    double pval=0.0;
								     if (dist<0.5) pval=1.0;
								     else if (dist<1.41) pval=1.41-dist;

								     m_out.dat.set(zi,yi,xi,m_out.dat.get(zi,yi,xi)+val*pval);
								 }
							 }
						 }
					 }
