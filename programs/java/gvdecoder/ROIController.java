	 ...
	 public String ROIListing(){
      ...
      public void actionPerformed(ActionEvent e) {
		      if (e.getActionCommand().equals("delete")){
				  ...
				  if ((i>-1)&&(i<data.size())){
					  ROI roi=(ROI)data.elementAt(i);
					  Viewer2 vw=roi.vi;
					  vw.jp.rois.remove(roi);
...
class ROITableModel extends AbstractTableModel {
        ...
        public String getColumnName(int col) {
       	  ...
       	  else if (col==2) return "center";
       	  else  if (col==3) return "pixels";
       	  else  if  (col==4) return "value";
       	  else  if (col==5) return  "average";
       	  else  if (col==6) return "diameter";
       	  return "";
        }
		...
		else if (col==3) val= new Integer(roi.findAllPixels(roi.vi.X_dim, roi.vi.Y_dim));
		else if (col==4) val= new Integer(roi.sumAllPixels(roi.vi.X_dim, roi.vi.Y_dim));
        else if (col==5) val= new Double(roi.getAverageVal(roi.vi.X_dim, roi.vi.Y_dim));
        else if (col==6) val= new Double(roi.getDiameter());
        else if (col==2) val=new Point(roi.getCenter());
