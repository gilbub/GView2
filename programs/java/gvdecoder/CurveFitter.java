...
public class CurveFitter {
    ...
    public static final double alpha = -1.0;	  // reflection coefficient
    public static final double beta = 0.5;	  // contraction coefficient
    public static final double gamma = 2.0;	  // expansion coefficient
    public static final double root2 = 1.414214; // square root of 2

    public int fit;                // Number of curve type to fit
    public double[] xData, yData;  // x,y data to fit
    public int numPoints;          // number of data points
    public int numParams;          // number of parametres
    public int numVertices;        // numParams+1 (includes sumLocalResiduaalsSqrd)
    public int worst;			// worst current parametre estimates
    public int nextWorst;		// 2nd worst current parametre estimates
    public int best;			// best current parametre estimates
    public double[][] simp; 		// the simplex (the last element of the array at each vertice is the sum of the square of the residuals)
    public double[] next;		// new vertex to be tested
    public int numIter;		// number of iterations so far
    public int maxIter; 	// maximum number of iterations per restart
    public int restarts; 	// number of times to restart simplex after first soln.
    public double maxError;     // maximum error tolerance
...
*/
    ...
    public String showSimplex(int iter) {
        //ij.IJ.write("" + iter);
        for (int i = 0; i < numVertices; i++) {
        //    String s = "";
            for (int j=0; j < numVertices; j++)
