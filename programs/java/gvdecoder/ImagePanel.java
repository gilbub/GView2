...
public BufferedImage getImage(){
	BufferedImage bn=new BufferedImage((int)(X_dim*scale),(int)(Y_dim*scale),BufferedImage.TYPE_INT_RGB);
	Graphics2D g2d=bn.createGraphics();
	paintComponent(g2d);
	return bn;
	}

public boolean saveImage(String fullpath){
	try{
 ...
 addKeyListener(new KeyListener(){
         ...
         public void keyReleased(KeyEvent e){
 			...
 			if (e.isControlDown()&&(e.getKeyCode() == KeyEvent.VK_C)){
 		     gvdecoder.ImageUtils.setClipboard(returnImage());
 		     AnalysisHelper.getAnalysisHelper().Notify("view exported to clipboard");
 		    }
 		 }
...
public void paintComponent(Graphics g) {
  ...
  public void drawOverlay(Graphics2D g, Decoration[] decorations){
	 ...
	 for (int i=0;i<decorations.length;i++){
		 ...
		 int sx=dec.xloc;
		 int sy=dec.yloc;
		 if (dec.absolute==false){
		   sx=(int)((float)dec.xloc*scale)+offsetx;
		   sy=(int)((float)dec.yloc*scale)+offsety;
		  }
		 if (dec.str!=null){
			 g.setColor(dec.color);
			 if (dec.fontsize!=0) g.setFont(new Font(dec.fontname,Font.PLAIN, dec.fontsize));
			 g.drawString(dec.str,sx,sy);
		 }
		 if (dec.ruler!=null){
		   g.setColor(dec.color);
		 ...
		 if (dec.shape!=null){
             ...
             g.setStroke(new BasicStroke(dec.linewidth, BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND));
