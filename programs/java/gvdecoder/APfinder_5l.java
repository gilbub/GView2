...
public class APfinder_5l{
 ...
 public APfinder_5l(Matrix ma, int apd, int upstrokeduration, int plateauduration, int minperiod, int maxperiod ){
	 this.m_in=ma;
	 this.APD=apd;
...
public void upsample(doubleArray3D d_in, doubleArray3D d_out, int z_in, int z_out, double out_state, double in_state, double thresh, int r){
	...
	for (int y=0;y<d_in.ydim;y++){
		for (int x=0;x<d_in.xdim;x++){
			...
			if ((d_out.arr[index_out]==0)||(d_out.arr[index_out]>in_state)){//&&(d_in.arr[index_in]==out_state)){
