...
public class ContourPlot extends Canvas {
	...
	void GetExtremes() {
		...
		if (zMin == zMax) {
			InvalidData();
			System.out.println("INVALID DATA zmin=zmax");
		}
		return;
	}
	...
	void AssignContourValues(){
		...
		if ((logInterpolation) && (zMin <= 0.0)) {
			InvalidData();
			System.out.println("need all data>0 for log interp");
		}
		if (logInterpolation) {
	...
	public void AddRow() {
		...
		if (leng >= MAX_X_STEPS)
			System.out.println("data to long");
		...
		throws IOException
	{
		...
		if (t.ttype != c) {
			InvalidData();
			System.out.println("error skipping blanks");
		}
	}
	public void DrawTheContourPlot(float [][] arr) {
				z=arr;
	...
	public static void main(String [] arg){
		float[][] dat=new float[10][10];
		for (int i=0;i<10;i++){
			for (int j=0;j<10;j++){
				 dat[i][j]=i*j;
			 }
		 }
		 ContourPlot cp=new ContourPlot(10,10);
