 ...
 **/
public class ImageJ_16bitAdapter implements ImageDecoder{
   ...
   public ImageJ_16bitAdapter(int x, int y, int frames, Object[] p, boolean isSigned){
   myxdim=x;
   myydim=y;
   ...
   public  int UpdateImageArray(int[] arr,int xdim,int ydim, int instance){
	   ...
	   for (int y=0;y<ydim;y++){
		   for (int x=0;x<xdim;x++){
			  v=(int)(((short[])pixelarray[FrameNumber])[y*xdim+x]);
			  if (!signed){
				  if (v<0) v=65536+v;
			  }else
			   v-= 32768;
