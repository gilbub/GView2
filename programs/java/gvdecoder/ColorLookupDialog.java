...
public class ColorLookupDialog extends JDialog {
	...
	for (int i=0;i<lookup[0].length;i++){
	 ...
	 lookup[1][i]=(int)tmp2[1][i];
	 lookup[2][i]=(int)tmp3[1][i];
	 if (lookup[0][i]<0) lookup[0][i]=0;
	 if (lookup[0][i]>255) lookup[0][i]=255;
	 if (lookup[1][i]<0) lookup[1][i]=0;
	 if (lookup[1][i]>255) lookup[1][i]=255;
	 if (lookup[2][i]<0) lookup[2][i]=0;
	 if (lookup[2][i]>255) lookup[2][i]=255;
	 }
    ...
    private ColorLookupDialog(Frame frame, int[] data, int[] hist, String title,
		...
		p3.setEditable(0);
		        ...
		        for (int i = 0; i < lookup[0].length; i++) {
		            ...
		            p2.addPoint(1, (double)i,(double)hist[i],!first);
		            p3.addPoint(0, (double)i,(double)lookup[0][i], !first);
		            p3.addPoint(1, (double)i,(double)hist[i],!first);
		            first = false;
    ...
    public static void main(String[] args) {
        ...
        button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                for (int j=0;j<tmpdat[0].length;j++) System.out.println(tmpdat[0][j]+" "+tmpdat[1][j]+" "+tmpdat[2][j]);
