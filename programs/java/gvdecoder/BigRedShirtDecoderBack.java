...
public int UpdateImageArray(int[] arr, int xdim, int ydim, int instance){
    ...
    */
    ...
    try{
     for (int x=0;x<xdim;x++){
	  for (int y=0;y<ydim;y++){
		int index=5120+  2*zdim*(x*xdim+y) + FrameNumber*2;
		Picfile.seek(index);
	    arr[x*xdim+y]=getRedShirtShort(Picfile);
	  }
    }
    FrameNumber++;
     ...
     for (int i=0;i<rois.length;i++){ //for each roi
       ...
       for (int j=0;j<rois[i].length;j++){ //go to each element in the roi
         ...
         for (int z=startframe;z<endframe;z++){
          ...
          int index=5120+  2*numframes*(yindex*framewidth+yindex)+z*2;
		  Picfile.seek(index);

         sum[z-startframe][i]+=getRedShirtShort(Picfile);
