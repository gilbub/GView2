 ...
 */
 ...
 public CAHelper4(Matrix data, Matrix ca, int A, int R){
  initializemasks();
  this.ca=ca;
  this.data=data;
  this.A=A;
  this.R=R;
  buffer=new Matrix(1,ma.ydim,ma.xdim);
 }
...
public void inplane_fill(int zindex,int r,double thr,double nbtrh, int setto){
   buffer.dat.set(0);
   for (int y=0;y<ca.ydim;y++){
	   for (int x=0;x<ca.xdim;x++){
...
publc double[] ratio(Matrix ma, int z, int y0, int x0, int r, double thr){
	int circleweight=0;
