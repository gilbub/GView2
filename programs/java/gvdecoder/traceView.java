...
if ( g2==null){
	...
	for (int i=0;i<16;i++){
		for (int j=0;j<maxyindex;j++){
			rendersingle(g2,i,j,topx,topy,(int)(scaledwidth),(int)(scaledheight));
...
public void render(int xs, int ys, int xe, int ye, int width, int height){
    ...
    if (traces!=null){
	 for (int i=xs;i<xe;i++){
		 for (int j=ys;j<ye;j++){
		    rendersingle(g2,i,j,(float)xs,(float)ys,width,height);
		 }
	 }
...
public void rendersingle(Graphics2D g2, int i, int j, float xs, float ys, int width, int height){
    ...
    if ((GlobalTimes!=null)&&(GlobalTimes.size()>0)){
		...
		g2.draw(getTimes(GlobalTimes,i,j,xs,ys,width,height));
	}
	if (cursormode){
		if (CursorTimes!=null){
    ...
    if (voltage) {
	 retranslate(i,j,xs,ys,width,height);
...
public void populate(){
	...
	traces=new singleTrace[16][ypixels];
	for (int x=0;x<16;x++){
		for (int y =0;y<ypixels;y++){
			...
			tr.gp=m.getGeneralPath(x,y,0,800,width,height);
